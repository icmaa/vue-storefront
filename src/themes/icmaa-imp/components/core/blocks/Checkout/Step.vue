<template>
  <div class="step t-relative" :class="[ 'step-' + name ]">
    ...
    <div class="t-flex t-items-center t-relative" :class="{ 't-pb-4': !last }">
      <div
        ...
        :class="{ 't-text-base-tone': active }"
      />
      <h2 class="t-flex-auto t-font-light t-text-xl" :class="[ active ? '' : 't-text-base-lighter' ]">
        <slot name="header">
          {{ title || name }}
        </slot>
      </h2>
    </div>
    <div v-if="active" class="t-pl-12 t-pb-8">
      <slot />
    </div>
  </div>
</template>
