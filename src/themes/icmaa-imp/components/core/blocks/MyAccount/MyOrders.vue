          ...
          <div :order="order" :key="order.entity_id" class="t-flex t-flex-wrap t-items-center t-cursor-pointer t-rounded-sm t-bg-base-lightest t-mb-4 t-px-2 t-py-3" @click="redirect(order.entity_id)">
            <div class="t-flex t-flex-wrap ">
              <div class="t-w-full t-flex t-justify-between">
                <div>{{ $t('Order #') }}: {{ order.increment_id }}</div>
                <strong><div>{{ order.grand_total | price }}</div></strong>
              </div>
              <div class="t-w-full">
                {{ $t('Order date') }}: {{ order.created_at | date }}
              </div>
            </div>
            <div class="t-flex t-flex-wrap ">
              <div class="t-w-full sm:t-w-1/2 t-p-2 t-flex t-items-center">
                <status-icon :status="order.status" /><span v-show="viewport !== 'xs'" class="status">&nbsp;{{ order.status | capitalize }}</span>
              </div>
            </div>
