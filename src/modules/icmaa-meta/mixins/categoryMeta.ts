...
export default {
    ...
    categoryImages () {
      ...
      this.getCategoryProducts.slice(0, limit).forEach(image => {
        categoryImagesTags.push({
          property: 'og:image',
          content:
            getThumbnailPath('/catalog/product' + image.image, this.width, this.height, 'media')
        });
      });
  ...
  metaInfo () {
    return {
      meta: [
        { vmid: 'og:title', property: 'og:title', content: htmlDecode(this.getCurrentCategory.name) },
        { vmid: 'og:url', property: 'og:url', content: htmlDecode(this.categoryUrl) },
